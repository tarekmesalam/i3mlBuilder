...
class AdminCronjobController extends Controller
{
    ...
    public function trigger(Request $request): JsonResponse
    {
        ...
        if ($command === 'yookassa:process-renewals' && ! \App\Models\Plugin::where('slug', 'yookassa')->where('status', 'active')->exists()) {
            return response()->json([
                'success' => false,
        ...
        return collect($this->getJobs())
            ->filter(function ($job) {
                ...
                if ($job['command'] === 'yookassa:process-renewals') {
                    return \App\Models\Plugin::where('slug', 'yookassa')->where('status', 'active')->exists();
                }
